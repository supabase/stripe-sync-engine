...
describe('parseNdjson', () => {
  it('parses a multi-line NDJSON string', async () => {
    const text = '{"a":1}\n{"b":2}\n{"c":3}'
    expect(await collect(parseNdjson(text))).toEqual([{ a: 1 }, { b: 2 }, { c: 3 }])
  })

  it('skips empty lines', async () => {
    const text = '{"a":1}\n\n{"b":2}\n'
    expect(await collect(parseNdjson(text))).toEqual([{ a: 1 }, { b: 2 }])
  })

  it('returns empty for blank string', async () => {
    expect(await collect(parseNdjson(''))).toEqual([])
  })
})
...
describe('parseNdjsonChunks', () => {
  ...
  it('handles a line split into many single-character chunks', async () => {
    const line = '{"x":42}\n'
