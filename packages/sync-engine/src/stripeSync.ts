...
export class StripeSync {
  ...
  private async fetchOnePage(
    ...
    try {
      ...
      // Build list parameters using starting_after for proper cursor-based pagination
      const listParams: Stripe.PaginationParams & { created?: Stripe.RangeQueryParam } = { limit }
      if (cursor && !/^\d+$/.test(cursor)) {
        // Non-numeric cursor = Stripe object ID, use starting_after
        listParams.starting_after = cursor
      } else if (config.supportsCreatedFilter && params?.created) {
        // Explicit created filter passed by caller
        listParams.created = params.created
      }
      ...
      if (response.data.length > 0) {
        ...
        // Update cursor with last record's ID for proper pagination
        const lastRecord = response.data[response.data.length - 1] as { id?: string }
        if (lastRecord?.id) {
