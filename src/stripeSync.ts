...
import { StripeSyncConfig } from './utils/config'
...
export class StripeSync {
  ...
  async syncSetupIntents(syncParams?: SyncBackfillParams): Promise<Sync> {
    logger.info('Syncing setup_intents')
  ...
  async syncPaymentIntents(syncParams?: SyncBackfillParams): Promise<Sync> {
    logger.info('Syncing payment_intents')
  ...
  async syncPaymentMethods(syncParams?: SyncBackfillParams): Promise<Sync> {
    ...
    logger.info(`Getting payment methods for ${customerIds.length} customers`)
  ...
  private async backfillCreditNotes(creditNoteIds: string[]) {
    const missingIds = await this.postgresClient.findMissingEntries('credit_notes', creditNoteIds)

    await this.fetchMissingEntites(missingIds, (id) => this.stripe.creditNotes.retrieve(id)).then(
      (creditNotes) => this.upsertCreditNotes(creditNotes)
    )
  }
