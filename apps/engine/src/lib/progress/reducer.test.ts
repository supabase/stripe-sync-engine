...
describe('createInitialProgress', () => {
  ...
  it('restores streams from prior progress', () => {
    const prior: ProgressPayload = {
      ...
      streams: {
        customers: { status: 'started', state_count: 2, record_count: 500, completed_ranges: [{ gte: '2024-01', lt: '2024-06' }] },
        invoices: { status: 'completed', state_count: 1, record_count: 100 },
      },
    }
    const p = createInitialProgress(prior)
    expect(p.streams['customers']).toMatchObject({ status: 'started', record_count: 500, completed_ranges: [{ gte: '2024-01', lt: '2024-06' }] })
...
describe('progressReducer — records', () => {
  it('counts records by stream', () => {
    ...
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: { id: '1' }, emitted_at: '2024-01-01T00:00:00.000Z' } })
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: { id: '2' }, emitted_at: '2024-01-01T00:00:00.000Z' } })
    p = progressReducer(p, { type: 'record', record: { stream: 'invoices', data: { id: '1' }, emitted_at: '2024-01-01T00:00:00.000Z' } })
  ...
  it('initializes stream entry on first record', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' } })
  ...
  it('does not mutate original state', () => {
    const p = createInitialProgress()
    const next = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' } })
...
describe('progressReducer — source_state', () => {
  ...
  it('does not create stream entry for global source_state', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'source_state', source_state: { state_type: 'global', data: { cursor: 'x' } } })
  ...
  it('does not mutate original state', () => {
    const p = createInitialProgress()
    const next = progressReducer(p, { type: 'source_state', source_state: { state_type: 'stream', stream: 'customers', data: {} } })
    expect(p.global_state_count).toBe(0)
...
describe('progressReducer — stream_status', () => {
  ...
  it('maps skip → skipped', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'skip', reason: 'not available' } })
    expect(p.streams['customers']?.status).toBe('skipped')
  })

  it('maps error → errored', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'error', error: 'forbidden' } })
  ...
  it('handles multiple streams independently', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'start' } })
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'invoices', status: 'complete' } })
  ...
  it('does not mutate original state', () => {
    const p = createInitialProgress()
    const next = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'start' } })
    expect(p.streams['customers']).toBeUndefined()
...
describe('progressReducer — connection_status', () => {
  it('sets connection_status', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'connection_status', connection_status: { status: 'failed', message: 'invalid key' } })
    expect(p.connection_status).toEqual({ status: 'failed', message: 'invalid key' })
  })

  it('does not mutate original state', () => {
    const p = createInitialProgress()
    progressReducer(p, { type: 'connection_status', connection_status: { status: 'failed', message: 'x' } })
    expect(p.connection_status).toBeUndefined()
  })
})
...
describe('progressReducer — derived.status', () => {
  ...
  it('is failed even if other streams succeeded', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'complete' } })
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'invoices', status: 'error', error: 'x' } })
    expect(p.derived.status).toBe('failed')
  })
  ...
  it('is started when some streams are still active', () => {
    let p = createInitialProgress()
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'customers', status: 'complete' } })
    p = progressReducer(p, { type: 'stream_status', stream_status: { stream: 'invoices', status: 'start' } })
    expect(p.derived.status).toBe('started')
  })
})
...
describe('progressReducer — elapsed_ms and rates', () => {
  ...
  it('computes records_per_second from record_count and elapsed', () => {
    let p = createInitialProgress()
    p = { ...p, started_at: '2024-01-01T00:00:00.000Z' }
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' }, _ts: '2024-01-01T00:00:02.000Z' })
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' }, _ts: '2024-01-01T00:00:02.000Z' })
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' }, _ts: '2024-01-01T00:00:02.000Z' })
    p = progressReducer(p, { type: 'record', record: { stream: 'invoices', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' }, _ts: '2024-01-01T00:00:02.000Z' })
  ...
  it('computes states_per_second from global_state_count and elapsed', () => {
    let p = createInitialProgress()
    p = { ...p, started_at: '2024-01-01T00:00:00.000Z' }
    p = progressReducer(p, { type: 'source_state', source_state: { state_type: 'stream', stream: 'customers', data: {} }, _ts: '2024-01-01T00:00:04.000Z' })
    p = progressReducer(p, { type: 'source_state', source_state: { state_type: 'stream', stream: 'customers', data: {} }, _ts: '2024-01-01T00:00:04.000Z' })
    // 2 states in 4 seconds = 0.5 sps
    expect(p.derived.states_per_second).toBe(0.5)
  })

  it('falls back to existing elapsed_ms when _ts is not set', () => {
    let p = createInitialProgress()
    p = { ...p, elapsed_ms: 1000 }
    p = progressReducer(p, { type: 'record', record: { stream: 'customers', data: {}, emitted_at: '2024-01-01T00:00:00.000Z' } })
    expect(p.elapsed_ms).toBe(1000)
  })
})
...
describe('progressReducer — unhandled messages', () => {
  it('returns same reference for log messages', () => {
    const p = createInitialProgress()
    expect(progressReducer(p, { type: 'log', log: { level: 'info', message: 'hi' } })).toBe(p)
  })

  it('returns same reference for catalog messages', () => {
    const p = createInitialProgress()
    expect(progressReducer(p, { type: 'catalog', catalog: { streams: [] } } as any)).toBe(p)
  })
})
