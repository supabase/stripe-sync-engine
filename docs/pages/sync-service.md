...
What the engine receives. Credentials inlined, state passed separately. This is the `SyncParams` type from `@tx-stripe/stateless-sync`:
...
> **Note on `createEngine()`**: `StatefulSync` uses `createEngine()` directly (from `@tx-stripe/stateless-sync`). The engine is the real interface — `StatefulSync` adds only the store-loading and state-persistence wrapper around it.
