...
`source-stripe` library code (`src/index.ts` entrypoint) has zero imports from `@tx-stripe/destination-postgres`.
...
- No `optionalDependencies` on `@tx-stripe/destination-postgres`
...
There is no `Orchestrator` interface — orchestration is handled by `createEngine()` in `@tx-stripe/stateless-sync`.
...
- [x] `createEngine()` in `@tx-stripe/stateless-sync`
