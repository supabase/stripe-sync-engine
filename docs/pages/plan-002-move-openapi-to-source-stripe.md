...
Change the openapi import from `../openapi` to `@stripe/sync-source-stripe`:
...
sync-engine already depends on `@stripe/sync-source-stripe` (package.json line 46).
...
- `packages/sync-engine/src/database/__tests__/migrate.openapi.test.ts` — change fixture import from `../../openapi/__tests__/fixtures/minimalSpec` to `@stripe/sync-source-stripe` (or a test util)
